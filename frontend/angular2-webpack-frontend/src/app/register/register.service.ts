...
export class RegisterService {
  ...
  private registerURL = 'http://localhost:9000/register';  // URL to web api
  ...
  public addPost (post: RegisterPost): Observable<RegisterPost> {
        let headers = new Headers({'Content-Type': 'application/json'});
        let options = new RequestOptions({headers});

        return this.http.post(this.registerURL, { post }, options)
            .map(this.parseData)
            .catch(this.handleError);
    }
